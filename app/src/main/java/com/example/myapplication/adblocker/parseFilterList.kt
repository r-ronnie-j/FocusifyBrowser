...
class ParseFilterListHelper {
    fun parseFilterList(
        ...
        val thirdPartyDomainRegularExpressionBlockList: MutableList<Array<String>> =
            ArrayList()  // 22.
        ...
        try {
            ...
            val bufferedReader =
                BufferedReader(InputStreamReader(assetManager.open(filterListName)))
            ...
            bufferedReader.forEachLine {
                ...
                // Parse the entry.
                if (filterListEntry.contains("##") || filterListEntry.contains("#?#") || filterListEntry.contains(
                        "#@#"
                    ) || filterListEntry.startsWith("[")
                ) {
                ...
                } else if (filterListEntry.contains("\$websocket") || filterListEntry.contains("\$third-party,websocket") || filterListEntry.contains(
                        "\$script,websocket"
                    )
                ) {
                    ...
                    if (filterListEntry.contains("$")) {  // The entry contains a filter option.
                        if (filterListEntry.contains("~third-party")) {  // Ignore entries that contain `~third-party`.
                            ...
                            if (filterListEntry.contains("domain=")) {  // Third-party domain allowed entries.
                                ...
                                val filters =
                                    filterListEntry.substring(filterListEntry.indexOf("$") + 1)
                                    ...
                                    if (entry == "http://" || entry == "https://") {  // Ignore generic entries.
                                        ...
                                        do {
                                            ...
                                            if (entry.contains("*")) {  // Process a third-party domain initial allowed double entry.
                                                ...
                                                thirdPartyDomainInitialAllowList.add(
                                                    domainDoubleEntry
                                                )
                                                ...
                                                val domainEntry =
                                                    arrayOf(domain, entry, originalFilterListEntry)
                                ...
                                if (entry.contains("*")) {  // There are two or more entries.
                                    ...
                                    if (secondEntry.contains("*")) {  // There are three or more entries.
                                        ...
                                        // Split the entry into components.
                                        val realSecondEntry =
                                            secondEntry.substring(0, secondWildcardIndex)
                                        val thirdEntry =
                                            secondEntry.substring(secondWildcardIndex + 1)
                                        ...
                                        if (thirdEntry.contains("*")) {  // There are four or more entries.
                                            ...
                                            // Split the entry into components.
                                            val realThirdEntry =
                                                thirdEntry.substring(0, thirdWildcardIndex)
                                            val fourthEntry =
                                                thirdEntry.substring(thirdWildcardIndex + 1)
                                            ...
                                            if (fourthEntry.contains("*")) {  // Process a third-party allow list quintuple entry.
                                                ...
                                                // Split the entry into components.
                                                val realFourthEntry =
                                                    fourthEntry.substring(0, fourthWildcardIndex)
                                                val fifthEntry =
                                                    fourthEntry.substring(fourthWildcardIndex + 1)

                                                // Create an entry string array.
                                                val quintupleEntry = arrayOf(
                                                    firstEntry,
                                                    realSecondEntry,
                                                    realThirdEntry,
                                                    realFourthEntry,
                                        ...
                                        if (entry.contains("*")) {  // There are two or more entries.
                                            ...
                                            if (secondEntry.contains("*")) {  // There are three or more entries.
                                                ...
                                                if (thirdEntry.contains("*")) {  // Process a domain allow list quadruple entry.
                                                    ...
                                                    // Create an entry string array.
                                                    val domainQuadrupleEntry = arrayOf(
                                                        domain,
                                                        firstEntry,
                                                        realSecondEntry,
                                                        realThirdEntry,
                        ...
                        if (entry.contains("*")) {  // Process a final allow list double entry
                            ...
                            // Create an entry string array.
                            val doubleEntry =
                                arrayOf(firstEntry, secondEntry, originalFilterListEntry)
                    ...
                    if (filterListEntry.contains("third-party")) {  // Third-party entries.
                        ...
                        if (filterListEntry.contains("~third-party")) {  // Third-party filter allow list entries.
                            ...
                            if (filterListEntry.startsWith("|")) {  // Third-party domain initial entries.
                                    ...
                                    do {
                                        ...
                                        if (entry.contains("*")) {  // Three are two or more entries.
                                            ...
                                            if (secondEntry.contains("*")) {  // Third-party domain initial block list triple entry.
                                                ...
                                                // Add the entry to the block list.
                                                thirdPartyDomainInitialBlockList.add(
                                                    tripleDomainEntry
                                                )
                                ...
                                val entry =
                                    filterListEntry.substring(0, filterListEntry.indexOf("/$") + 1)
                                val filters =
                                    filterListEntry.substring(filterListEntry.indexOf("/$") + 2)
                                var domains = filters.substring(filters.indexOf("domain=") + 7)
                            ...
                            // Get the entry.
                            val entry =
                                if (filterListEntry.contains("$/$"))  // The first `$` is part of the regular expression.
                    ...
                    } else if (filterListEntry.substring(filterListEntry.indexOf("$"))
                            .contains("domain=")
                    ) {  // Domain entries.
                        // Process the entry.
                        ...
                        if (filterListEntry.endsWith("*"))
                            filterListEntry =
                                filterListEntry.substring(0, filterListEntry.length - 1)
